...
const assets=Object.fromEntries(['prepared/manifest.json','vendor/maplibre-gl.js','route-detail.json','route-links.json','moments.json','journey-style.json','elevation-profile.json'].map(file=>[file,createHash('sha256').update(readFileSync(new URL('public/trek/'+file,root))).digest('hex').slice(0,12)]));
const data={assets,landmarks,countryRings:read('data/trek-atlas.json').countries,days,photos,scenes,timeline,total:metrics.total.km,recorded:metrics.recorded,estimated:metrics.estimated,colors:Object.fromEntries(source.countries.map(c=>[c.name,c.color])),stats:{days:walked,ascent:metrics.total.ascent,minutes,countries:source.facts.countries}};
